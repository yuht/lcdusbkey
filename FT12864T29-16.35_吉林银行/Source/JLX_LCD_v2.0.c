...
//unsigned char xdata InitCmd[]={0x2F,0x25,0x81,0x10,0xA1,0xA0,0xC8,0x10,0x00,0xB0,0xAF,0xA6,0x40};
unsigned char xdata InitCmd[]={0x2F,0x22,0x81,0x3F,0xA2,0xA1,0xC0,0x10,0x00,0xB0,0xAF,0xA6,0x40};
...
void InitLCD()
{
	unsigned char InitCmdLength = sizeof(InitCmd);
	unsigned char i;
	
	SDAT  = SCLK =CD =RS =CS =LCD_LED = 0;

    timer(100);//复位前的延时必须大于3ms    
    RS = 1;
	timer(100);//复位前的延时必须大于3ms    
      
	for(i=0;i<InitCmdLength;i++)
	{
		wlc(InitCmd[i]);
		timer(100);
	}
