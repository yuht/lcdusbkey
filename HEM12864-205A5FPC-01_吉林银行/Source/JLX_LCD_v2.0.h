...
sbit SDAT = P0 ^ 7;	//LCD pin8  数据
sbit SCLK = P0 ^ 6;	//LCD pin9  时钟
sbit CD = P0 ^ 5;	//LCD pin10 寄存器选择，高电平传输数据，低电平传输指令
sbit RS = P5 ^ 3;	//LCD pin11 复位，低电平复位，复位完成后，回到高电平
sbit CS = P6 ^ 5;	//LCD pin12 片选，低电平片选
...
void InitLCD();//初始化液晶屏
void CleanLCD(unsigned char dat);//清屏
